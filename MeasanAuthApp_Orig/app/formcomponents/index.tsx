...
export default ({
  ...
  React.useEffect(() => {
    mFormData.map(k => {
      if (k.fieldType !== 'input') register(k.name, validation[k.name]);
    });
  }, [register]);
      ...
      <KeyboardAwareScrollView
        ...
        <View style={styles.formContainer}>
          {mFormData.map(element => {
            if (element.fieldType === 'input' && element.name === 'password')
              return (
                <Form
                  {...{register, setValue, validation, errors}}
                  key={element.name}>
                  <Input
                    name={element.name}
                    label={element.label}
            ...
            else if (element.fieldType === 'input')
              return (
                <Form
                  {...{register, setValue, validation, errors}}
                  key={element.name}>
          ...
          <Button title={button_title} onPress={handleSubmit(onSubmit)} />
